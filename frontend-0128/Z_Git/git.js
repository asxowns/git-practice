...
- 사용자 이름과 이메일 설정
  ...
  : MacOs (Git Bash, 터미널)
  >> 위 환경의 붙여넣기 단축기 : shift + Ins
  >> 새로운 명령어 환경 열기 - Ctrl + c
  >> 방향키(위/아래): 이전 또는 앞전의 명령어 붙여넣기

  ? 사용자 이름 설정
  git config --global user.name 본인 깃허브 아이디
  ex) git config --global user.name asxowns

  ? 사용자 이메일 주소 설정
  git config --global user.email 깃허브 이메일
  ex) git config --global user.email 1000pgm@gmail.com
  ...
  2. staging area: 커밋하기 전에 파일의 변경사항을 준비하는 장소,
    기존의 로컬 스토리지와의 변경사항을 컴토하고 커밋할 항목을 결정

  3. local repository: 사용자의 개인 컴퓨터에 위치한 저장소,
  개인 작업을 진행하고 저장

  4. remote repository: 인터넷이나 네트워크에 위치한 저장소,
  Github의 호스팅 서비스를 사용
  ...
  >> README.md 파일을 생성 (# git-practice 텍스트를 추가)
  ...
  git remote add origin https://github.com/asxowns/git-practice.git
  >> 리모트 저장소를 추가

  git push -u origin main
  >> 로컬 리포지토리 main 브랜치를 원격 리포지토리에 푸시
  >> -u: 추후 git push나 git pull을 진행 할 때 브랜치 이름을 생략
  가능하도록 설정
  ...
  2. workspace 에서 remote repository까지 파일 업로드

  git add .
  git commit -m "20240128 ktj"
  git push origin main
  ...
  3. 작업하는 환경(work space)과 원격 리포지토리를 연결
